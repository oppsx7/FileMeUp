<script type = "text/javascript" src = "./copyOnClipboard.js"></script>
<?php
session_start();
...
if(isset($_POST["action"]))
{
    ...
    // $query = "Select name FROM folders WHERE shared = 1";
    ...
    if($_POST["action"] == "create")
    {
        if(!file_exists($_POST["folder_name"]))
        {
            ...
            $query = "INSERT INTO folders(name, shared, path) VALUES('$folderName', '1', '$path')";
        ...
        foreach($file_data as $file)
        {
            ...
            else
            {
                ...
                <tr>
                    ...
                    <td><a id="'.$fileid.'" onclick="copyToClipboard(\'#'.$fileid.'\')" style="word-wrap: break-word;width:20px;">http://'.$_SERVER  ['HTTP_HOST']."/" . 'filemeup/' .$path.'</a>
    ...
    if($_POST["action"] == "delete")
    {
        ...
        $fname = end(explode("/", $_POST["folder_name"]));
        $query = "DELETE FROM folders WHERE name = '$fname'";
        $run = mysqli_query($conn,$query);
        if ($run) {
            $query2 = "DELETE FROM files WHERE path like '%$fname%'";
            $sec_run = mysqli_query($conn,$query2);
            if ($sec_run){
                if(rmdir($_POST["folder_name"]))
            {
                ...
                ob_end_clean();
                echo 'Error deleting from file structure';
            } 
            }
            else {
                ob_end_clean();
                echo 'SQL ERROR when deleting files';
            }
