...
if(isset($_POST["action"]))
{
    ...
    {
        $uid = $_SESSION['userid'];
        $path = 'user_folders' . '/' . $uid . '/' . '*';
        $folder = array_filter(glob($path), 'is_dir');
        $output = '
    ...
    if($_POST["action"] == "create")
    {
        if(!file_exists($_POST["folder_name"]))
        {
            ...
            $prev_path = "user_folders" . '/' . $userID;
            $path =  "user_folders" . '/' . $userID . '/' . $folderName;
            $query = "INSERT INTO folders(name, shared, userID, path) VALUES('$folderName', '0', '$userID', '$path')";
            $run = mysqli_query($conn,$query);
            if($run) {
                mkdir($prev_path, 0777, true);
